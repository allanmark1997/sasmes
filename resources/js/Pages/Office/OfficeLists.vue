...
const open_modal = (office) => {
    form_update.reset()
    post_image.value = []
    form_update.office = office
    form_update.name = office.name
    form_update.abbrevation = office.abbrevation
    post_image.value.push(office.photo)
    update_modal.value = !update_modal.value
}

const open_modal_delete = (office) => {
    form_delete.reset()
...
const update_office = () => {
    form_update.post(route("office.update", { office: form_update.office }), {
        preserveScroll: true,
        onSuccess: () => {
            form_update.reset();
            toast.success("Office has been successfully updated!", {
                autoClose: 1000,
                transition: toast.TRANSITIONS.FLIP,
                position: toast.POSITION.TOP_RIGHT,
            });
...
const confirm_delete = () => {
    form_delete.delete(route("office.destroy", { office: form_delete.office }), {
        preserveScroll: true,
        onSuccess: () => {
            form_delete.reset();
...
<template>
    <div v-if="offices.data.length == 0">
    ...
    <JetDialogModal :show="update_modal" @close="update_modal = false" maxWidth="2xl">
        <template #title>Update Office here!</template>
        <template #content>
            <div class="grid grid-cols-12 gap-2">
        ...
        <template #footer>
            ...
            <PrimaryButton :class="{ 'opacity-25': form_update.processing }" :disabled="form_update.processing"
                class="hover:bg-green-300" @click="update_office">
